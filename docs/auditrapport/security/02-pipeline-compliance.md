...
| **A.8.29 Beveiligingstests** | Software toetsen op beveiligingseisen. | Dependabot, CodeQL, Snyk, SBOM en Maven Tests ondersteunen security testing. De Maven workflow draait API-tests, omod security regressietests en JaCoCo coverage rapporten op pull requests. PR-checks tonen dat CodeQL, Snyk en code scanning groen draaien op een pull request. | `.github/workflows/maven-tests.yml`; `bewijs/scanning/maven-tests-coverage-artifacts-success.png`; `bewijs/scanning/jacoco-api-coverage-overview.png`; `bewijs/scanning/jacoco-omod-security-coverage-overview.png`; `bewijs/sbom-sca/dependabot-alerts-overview.png`; `bewijs/scanning/code-scanning-alerts.png`; `bewijs/scanning/pr-security-checks-passed.png`; `bewijs/sbom-sca/sbom-workflow-success.png` | Aanwezig; Maven workflow moet nog als required check worden geselecteerd |
...
- **Maven Tests en JaCoCo:** `bewijs/scanning/maven-tests-coverage-artifacts-success.png` toont een groene Maven Tests workflow met coverage artifacts; `bewijs/scanning/jacoco-api-coverage-overview.png` en `bewijs/scanning/jacoco-omod-security-coverage-overview.png` tonen de inhoudelijke coverage-overzichten.
...
| Required checks en minimaal 1 review zijn ingericht volgens teamwerkwijze. De screenshots tonen de actieve `protect-main` ruleset en dependency PR's met `Review required`. De Maven Tests workflow draait groen en uploadt coverage artifacts; deze check moet nog als required check worden toegevoegd. | A.8.3, A.8.29 | Gedeeltelijk aanwezig |
