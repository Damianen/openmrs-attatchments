...
| Required checks en minimaal 1 review zijn ingericht volgens teamwerkwijze. De screenshots tonen de actieve `protect-main` ruleset en dependency PR's met `Review required`. De nieuwe Maven Tests workflow moet na de eerste run nog als required check worden toegevoegd; coverage artifacts moeten daarna als bewijs worden opgeslagen. | A.8.3, A.8.29 | Gedeeltelijk aanwezig |
