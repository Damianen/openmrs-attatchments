...
- instellingen waarvoor repository-owner rechten nodig zijn, zoals het selecteren van nieuwe required checks in GitHub.
...
| A.8.5 Authenticatie en autorisatie | Accounts, rechten en approvals moeten aantoonbaar zijn. | Ongeautoriseerde repositorywijzigingen of te brede toegang tot patientdata. | MFA-bewijs, branch ruleset, production environment rules en PR-reviewplicht zijn vastgelegd. | `02-pipeline-compliance.md`; `bewijs/repository-access/mfa-aliriza-sari.png`; `bewijs/repository-access/mfa-damian-buskens.png`; `bewijs/repository-access/mfa-mohammed-ouaali.png`; `bewijs/repository-access/main-branch-ruleset.png`; `bewijs/repository-access/production-environment-rules.png`; `bewijs/repository-access/dependabot-prs-review-required.png` | Aanwezig. |
...
| A.8.25 Secure SDLC | Security moet onderdeel zijn van ontwerp, build, review en release. | Kwetsbare code kan zonder review of securitytests naar main gaan. | Branch protection/ruleset, PR review, CodeQL, Snyk, SBOM en Maven Tests workflow zijn ingericht. | `02-pipeline-compliance.md`; `10-coverage-quality-gate.md`; `.github/workflows/codeql.yml`; `.github/workflows/snyk.yml`; `.github/workflows/sbom.yml`; `.github/workflows/maven-tests.yml`; `bewijs/scanning/pr-security-checks-passed.png`; `bewijs/scanning/maven-tests-coverage-artifacts-success.png` | Aanwezig; Maven Tests moet door een owner nog als required check worden geselecteerd. |
| A.8.28 Secure coding | Onveilige file access, uploadvalidatie en parsing moeten worden voorkomen. | Path traversal, upload allowlist bypass, MIME mismatch en malformed base64. | Path normalization/root-checks, veilige default allowlist, extensie/MIME-validatie en robuuste base64validatie zijn toegevoegd en getest. | `04-threat-model.md`; `05-security-backlog.md`; `risk-assessment-report.md`; `docs/auditrapport/08-pentest.pdf`; `api/src/main/java/org/openmrs/module/attachments/obs/DefaultAttachmentHandler.java`; `omod/src/main/java/org/openmrs/module/attachments/rest/AttachmentResource.java`; `omod/src/test/java/org/openmrs/module/attachments/rest/AttachmentResourceTest.java`; `omod/src/test/java/org/openmrs/module/attachments/rest/AttachmentRestControllerTest.java` | Aanwezig voor de belangrijkste onderzochte codepaden. |
| A.8.29 Security testing | Belangrijke security-eisen moeten aantoonbaar getest worden. | Security regressies kunnen terugkomen zonder automatische testdekking. | Security regressietests draaien in Maven workflow; JaCoCo artifacts worden opgeslagen; pentestdocument is bijgewerkt met 10 cases. | `10-coverage-quality-gate.md`; `docs/auditrapport/08-pentest.pdf`; `bewijs/scanning/maven-tests-coverage-artifacts-success.png`; `bewijs/scanning/jacoco-api-coverage-overview.png`; `bewijs/scanning/jacoco-omod-security-coverage-overview.png`; `.github/workflows/maven-tests.yml` | Aanwezig; owneractie nodig om Maven Tests verplicht te maken voor merge. |
...
|---|---|---|
| Maven Tests als required check selecteren | Hiervoor zijn repository-owner rechten nodig. | Owner moet na een groene workflow-run de check toevoegen aan de branch/ruleset. |
...
De belangrijkste NEN-controls zijn traceerbaar naar concrete repositorybestanden, bewijsafbeeldingen, workflows, tests en het pentestdocument. De auditbasis is daarmee aanwezig. De resterende punten zijn vooral afhankelijk van repository-owner rechten of productie-runtimeinformatie buiten deze repository.
