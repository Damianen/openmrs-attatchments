...
| Download attachment bytes | Geeft patientbestanden terug. Onjuiste autorisatie leidt direct tot datalek. | Gemitigeerd met privilegecheck en regressietest; auditlogging nog beoordelen. |
| Search attachments | Geeft patientmetadata en attachmentmetadata terug op basis van requestparameters. | Inputvalidatie en logging nog verder beoordelen. |
...
| AS-03 | Attachment verwijderen | REST delete | `AttachmentResource.delete`, `AttachmentResource.java:96` | UUID en delete reason | OpenMRS REST auth, ObsService | Onterecht voiden/verwijderen van patientattachment | Ja | OpenMRS REST auth; logging/auditstatus nog beoordelen |
...
| AS-06 | Download attachment bytes | REST download | `AttachmentBytesResource.getBytes`, `AttachmentBytesResource.java:42` | Attachment UUID | OpenMRS auth, `Context.hasPrivilege`, ObsService, complex obs handler | IDOR, datalek van attachment bytes | Ja | Expliciete privilegecheck en regressietest aanwezig |
