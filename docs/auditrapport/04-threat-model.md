...
| T03 | Uploadvalidatie | Ongewenste bestandstypen kunnen worden geupload als allowlist leeg is. | Tampering, Denial of Service | Integriteit, beschikbaarheid | 4 | 4 | 16 | `config.xml` heeft lege `allowedFileExtensions`; `AttachmentResource.java` regels 131-170 checkt extensie/MIME alleen als allowlist gevuld is. | Veilige default allowlist instellen; MIME en extensie altijd valideren; tests toevoegen. |
...
| P2 | Verwijder PII uit applicatielogs en voeg veilige auditlogging toe. | T05 |
